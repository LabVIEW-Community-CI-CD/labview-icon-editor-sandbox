...
- Scope and warnings: explicitly limited to dev/test. Production must use CA-issued certs; self-signed artifacts shall not be promoted to prod.
...
- Cons: self-signed certs are not suitable for production; requires elevated PowerShell to write to LocalMachine store; backend binding still needs to be performed by the operator; requires maintaining and trusting an internal code-signing cert; requires hash/signature verification discipline.

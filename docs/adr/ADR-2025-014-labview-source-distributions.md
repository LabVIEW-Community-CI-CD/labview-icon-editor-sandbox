...
- **Future work**: If additional control is required (e.g., dynamic spec updates or multi-step build logic), the Application Builder API remains available. For most cases the CLI path will suffice, but we can revisit the more complex scripting approach if runtime specification changes are required.

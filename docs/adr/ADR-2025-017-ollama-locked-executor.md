...
- **B** — Remote cloud LLM executor (OpenAI/Azure, etc.) with similar wrapper; pros: no local model setup, higher-quality models; cons: data egress/cost, network dependency, harder to enforce allowlists and latency/timeout guarantees.
...
- **–** Guardrails must be maintained as scripts evolve; lock-step command changes require task and allowlist updates.
